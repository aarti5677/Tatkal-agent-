...
;(() => {
  ...
  function injectInterceptor() {
    ...
    keysNode.dataset.bypassKey = "PRO_MAX_CTF_2026"; // Inject the bypass key
  ...
  function abortFlow() {
    state.aborted = true;
    state.runToken++;
    if (state.captchaResolve) { state.captchaResolve(null); state.captchaResolve = null; }
    if (state.otpResolve) { state.otpResolve(null); state.otpResolve = null; }
    sessionStorage.removeItem(PENDING_KEY);
    panelStatus("Aborted");
  }
  ...
  async function ensureLoggedIn() {
    ...
    const open = findActionByText(/^login$/i, document, ["a", "button", "span", "div"]) || queryFirstVisible(SEL.login?.open);
    if (open) { fastClick(open); await minimalDelay(40); }
    ...
    const submit = findActionByText(/^(sign in|login|submit)$/i) || findEnabledAction(SEL.login?.submit, /login|sign in|submit/i);
  ...
  async function resolveLoginChallenges() {
    ...
    while (Date.now() < deadline) {
      checkAbort();
      if (isLoggedIn()) { log("Login confirmed.", "log-success"); return; }
      if (queryFirstVisible(SEL.captcha?.input)) {
        await handleCaptchaIfPresent();
        const submit = findActionByText(/^(sign in|login|submit|continue)$/i) || findEnabledAction(SEL.login?.submit, /login|sign in|submit|continue/i);
        if (submit) fastClick(submit);
      }
      // REAL OTP FIX: Actually wait for and handle the OTP input
      if (queryFirstVisible(SEL.otp?.input)) {
        await handleOtpIfPresent();
      }
      await waitUntil(() => isLoggedIn() || queryFirstVisible(SEL.captcha?.input) || queryFirstVisible(SEL.otp?.input), Math.min(5000, Math.max(250, deadline - Date.now())), "login challenge").catch(() => null);
    }
  ...
  async function fillSearchForm() {
    ...
    } else { log("Class control not found; continuing with visible defaults.", "log-warn"); }
    if (DOM.quota) {
      await selectControl(DOM.quota, quota, quotaAliases(quota));
      log("Quota set to " + quota, "log-success");
    } else { log("Quota control not found; continuing with visible defaults.", "log-warn"); }
  }

  async function fillAutocomplete(input, displayValue, codeValue) {
    // FIX: Prioritize station code (e.g., NDLS) over display name for speed
    const valueToType = String(codeValue || displayValue || "").trim();
    if (!input || !valueToType) return;
    ...
    fastFill(input, valueToType);
    await minimalDelay(50);

    // Hit ArrowDown and Enter to select the first dropdown option fast
  ...
  async function fillDate(input, yyyyMmDd) {
    ...
    fastClick(input);
    await minimalDelay(30);
    fastFill(input, uiDate);
    await minimalDelay(30);
    // FIX: Force close PrimeNG calendar so it doesn't block the Search button
    document.body.click();
    await minimalDelay(30);
  ...
  async function submitSearch() {
    const searchButton = await waitUntil(() => findActionByText(/^(search|find trains)$/i) || findEnabledAction(SEL.search?.submit, /search|find trains/i), CFG.ELEMENT_WAIT_MS || 15000, "search button");
  ...
  async function selectTargetTrainAndClass() {
    ...
    const trains = [targetTrain, ...(Array.isArray(state.journey.alternateTrains) ? state.journey.alternateTrains.map(String) : [])].map(s => s.trim()).filter(Boolean);
    const row = await waitUntil(() => {
      acceptDialogs().catch(() => null);
      for (const train of trains) { const found = findTrainRow(train); if (found) return found; }
      return null;
    ...
    await acceptDialogs();
    await waitUntil(() => location.href.includes("/booking/psgninput") || queryFirstVisible(SEL.passenger?.root) || queryFirstVisible(SEL.passenger?.name), CFG.PAGE_WAIT_MS || 25000, "passenger page");
  }

  function findTrainRow(trainNo) {
    if (!trainNo) return null;
    const rows = queryAll(SEL.results?.rows).filter(isVisible).sort((a, b) => textOf(b).length - textOf(a).length);
  ...
  async function selectClassAndCheckAvailability(row, classCode) {
    ...
    if (!classNode) throw new Error("Class " + classCode + " not found in train row.");
    fastClick(classNode);
    await minimalDelay(40);
    const refreshBtn = findActionByText(/check availability|availability|refresh/i, row) || findActionByText(/check availability|availability|refresh/i);
    ...
    if (!availableTile) throw new Error("Seats not available for class " + classCode);
    log("Seats available for " + classCode, "log-success");
    fastClick(classNode);
    await minimalDelay(40);
    const bookBtn = await waitUntil(() => findActionByText(/book now|book ticket|book|continue|select/i, row) || findActionByText(/book now|book ticket|book|continue/i), CFG.ELEMENT_WAIT_MS || 15000, "book button");
    fastClick(bookBtn);
  }
  ...
  async function fillPassengers() {
    ...
    await waitUntil(() => queryFirstVisible(SEL.passenger?.root) || queryFirstVisible(SEL.passenger?.name), CFG.PAGE_WAIT_MS || 25000, "passenger form");
    ...
    if (confirmBerthCheck) {
      ...
      if (!checkbox.checked && !checkbox.classList.contains('ui-state-active')) fastClick(checkbox);
    }
  }
  ...
  async function continueToPayment() {
    await handleCaptchaIfPresent();
    const firstContinue = findActionByText(/^(continue|next|review journey|proceed)$/i) || findActionByText(/continue|proceed|review/i, document, SEL.passenger?.continue || ["button", "a"]);
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      if (action && isEnabled(action)) { fastClick(action); await minimalDelay(50); }
      const ready = await waitUntil(() => isPaymentUiVisible() || queryFirstVisible(SEL.captcha?.input) || findActionByText(/proceed to payment|make payment|pay and book|pay & book/i), CFG.ELEMENT_WAIT_MS || 15000, "payment transition").catch(() => null);
  ...
  async function handlePaymentUi() {
    ...
    if (paytmUpi) fastClick(paytmUpi);
    else {
      const amazonUpi = findActionByText(/Amazon Pay/i, document, [".paymentOption", "label", "button", "div"]);
      if (amazonUpi) fastClick(amazonUpi);
    }
    await minimalDelay(40);
    const pay = findActionByText(/^(pay|make payment|proceed|continue)$/i, document, SEL.payment?.pay || ["button", "a"]) || findActionByText(/pay|make payment|proceed/i);
    if (pay && isEnabled(pay)) {
      fastClick(pay);
      log("Payment action clicked in UI. Complete any bank or UPI challenge manually.", "log-action");
    } else { log("Payment UI is visible. Complete payment manually if no safe pay button is available.", "log-warn"); }
    await watchBookingOutcome();
  }

  function isPaymentUiVisible() {
    const body = normalize(document.body?.innerText || "");
    if (/payment option|select payment|make payment|pay and book|payment gateway|upi|net banking|debit card|credit card/.test(body)) return true;
  ...
  async function watchBookingOutcome() {
    ...
    const outcome = await waitUntil(() => {
      ...
      if (pnr || /booking confirmed|congratulations|e-ticket|your ticket/.test(normalized)) return { ok: true, pnr };
      if (/booking failed|payment failed|not available|quota full|no seats|waitlist full|transaction failed/.test(normalized)) return { ok: false };
      return null;
    }, CFG.PAYMENT_WATCH_MS || 600000, "booking outcome").catch(() => null);
    if (!outcome) { log("Payment watch timed out. Check the IRCTC tab for final status.", "log-warn"); return; }
    if (outcome.ok) { safeSend({ action: "bookingSuccess", pnr: outcome.pnr || "" }); status("done", "BOOKED"); }
    else { safeSend({ action: "bookingError", reason: "Booking or payment failed" }); status("error", "FAILED"); }
  }
  ...
  async function handleCaptchaIfPresent() {
    ...
    if (state.settings.useAutoCaptcha && state.settings.twoCaptchaKey) {
      ...
      if (autoAnswer) { fastFill(input, autoAnswer); return true; }
    }
  ...
  async function handleOtpIfPresent() {
    const otpInput = queryFirstVisible(SEL.otp?.input);
    if (!otpInput) return false;

    log("OTP required by IRCTC. Waiting for manual entry...", "log-warn");
    status("booking", "ENTER OTP");
    
    // Send message to popup to show OTP input box
    safeSend({ action: "showOtpInput" });

    // Wait for user to input OTP (time out after 2 minutes)
    const otpValue = await waitForManualAnswer("otp", CFG.OTP_TIMEOUT_MS || 120000);
    if (!otpValue) throw new Error("OTP entry timed out.");

    fastFill(otpInput, otpValue);
    await minimalDelay(30);

    const submitOtp = findActionByText(/^(verify|submit|continue)$/i) || findEnabledAction(SEL.otp?.submit);
    if (submitOtp) fastClick(submitOtp);

    log("OTP submitted. Waiting for login confirmation...", "log-action");
  ...
  function waitForManualAnswer(type, timeoutMs) {
    return new Promise(resolve => {
      ...
      const timer = setTimeout(() => { if (state[key]) state[key] = null; resolve(null); }, timeoutMs);
      state[key] = answer => { clearTimeout(timer); state[key] = null; resolve(answer); };
    });
  }
  ...
  async function acceptDialogs() {
    ...
    if (accept && isEnabled(accept)) { fastClick(accept); await minimalDelay(30); }
  }
  ...
  async function selectControl(control, value, aliases = []) {
    ...
    if (control instanceof HTMLSelectElement) {
      ...
      if (option) control.value = option.value; else control.value = value;
      dispatchInputEvents(control);
      return;
    }
    if (isTextInput(control)) { await fillAutocomplete(control, labels[0], labels[0]); return; }
    ...
    if (option) { fastClick(option); await minimalDelay(20); }
  }
  ...
  function optionMatches(option, labels) {
    ...
    return labels.some(label => { const normalized = normalize(label); return normalized && (text === normalized || text.includes(normalized)); });
  }
  ...
  function fastFill(el, value) {
    ...
    el.focus?.();
    if (el.isContentEditable) { el.textContent = text; dispatchInputEvents(el); return; }
    if (el instanceof HTMLSelectElement) { el.value = text; dispatchInputEvents(el); return; }
  ...
  function dispatchInputEvents(el) {
    try { el.dispatchEvent(new InputEvent("input", { bubbles: true, inputType: "insertText" })); } catch { el.dispatchEvent(new Event("input", { bubbles: true })); }
  ...
  function waitForElement(selectors, timeoutMs) { return waitUntil(() => queryFirstVisible(selectors), timeoutMs, "element"); }

  function waitUntil(predicate, timeoutMs, label = "condition") {
    return new Promise((resolve, reject) => {
      let settled = false; let raf = 0;
      const timeout = setTimeout(() => finish(null, new Error("Timeout: " + label)), timeoutMs);
      const observer = new MutationObserver(check);
      function finish(value, error) { if (settled) return; settled = true; clearTimeout(timeout); cancelAnimationFrame(raf); observer.disconnect(); if (error) reject(error); else resolve(value); }
      function check() {
        if (settled) return;
        try { checkAbort(); const result = predicate(); if (result) finish(result); else raf = requestAnimationFrame(check); } catch (error) { finish(null, error); }
      }
  ...
  function queryFirst(selectors, root = document) { for (const selector of selectorList(selectors)) { try { const found = root.querySelector(selector); if (found) return found; } catch {} } return null; }
  function queryFirstVisible(selectors, root = document) { return queryAll(selectors, root).find(isVisible) || null; }
  function queryAllVisible(selectors, root = document) { return queryAll(selectors, root).filter(isVisible); }
  function queryAll(selectors, root = document) { const found = []; for (const selector of selectorList(selectors)) { try { found.push(...root.querySelectorAll(selector)); } catch {} } return uniqueElements(found); }
  function selectorList(selectors) { if (!selectors) return []; if (Array.isArray(selectors)) return selectors.flatMap(selectorList); return [String(selectors)]; }
  function uniqueElements(elements) { return Array.from(new Set(elements.filter(Boolean))); }

  function findEnabledAction(selectors, textPattern, root = document) {
    return queryAll(selectors, root).find(el => isVisible(el) && isEnabled(el) && (!textPattern || textPattern.test(textOf(el)))) || null;
  }
  ...
  function clickableAncestor(el) { return el?.closest?.("button,a,label,[role='button'],.p-dropdown,.ui-dropdown,.mat-select,.ng-select") || el; }
  function isVisible(el) { if (!el || !el.isConnected) return false; const style = getComputedStyle(el); if (style.display === "none" || style.visibility === "hidden" || Number(style.opacity) === 0) return false; const rect = el.getBoundingClientRect(); return rect.width > 0 && rect.height > 0; }
  function isEnabled(el) { if (!el) return false; const target = clickableAncestor(el) || el; return !target.disabled && target.getAttribute("aria-disabled") !== "true" && !target.classList.contains("disabled") && !target.classList.contains("ui-state-disabled") && !target.classList.contains("p-disabled"); }
  function isTextInput(el) { return el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement; }
  ...
  function classAliases(code) { const label = CFG.CLASS_LABELS?.[code] || ""; return [code, label, label ? label + " (" + code + ")" : ""].filter(Boolean); }
  function quotaAliases(code) { const label = CFG.QUOTA_LABELS?.[code] || ""; return [code, label, label ? label + " (" + code + ")" : ""].filter(Boolean); }
  function genderCode(gender) { const value = normalize(gender); if (value.startsWith("f")) return "F"; if (value.startsWith("t")) return "T"; return "M"; }
  function genderAliases(gender) { const code = genderCode(gender); const label = code === "F" ? "Female" : code === "T" ? "Transgender" : "Male"; return [code, label, gender].filter(Boolean); }
  function berthCode(berth) { const map = { "lower": "LB", "middle": "MB", "upper": "UB", "side lower": "SL", "side upper": "SU", "window": "WS", "no preference": "NP" }; return map[normalize(berth)] || "NP"; }
  function berthAliases(berth) { const code = berthCode(berth); return [code, berth || "No Preference", "No Preference"].filter(Boolean); }
  function paymentPattern(method) { const value = normalize(method || ""); if (!value) return null; if (value.includes("paytm")) return /paytm/i; if (value.includes("phonepe")) return /phonepe|phone pe/i; if (value.includes("razor")) return /razorpay/i; if (value.includes("ipay")) return /irctc\s*ipay|ipay/i; if (value.includes("upi")) return /upi|bhim/i; return new RegExp(escapeRegExp(method), "i"); }
  function extractPnr(text) { const match = /PNR(?:\s*(?:No|Number)\.?)?\s*[:\-]?\s*([0-9]{10})/i.exec(String(text || "")); return match?.[1] || ""; }
  function targetSummary() { return (state.target.train || state.journey.trainNo || "?") + " / " + (state.target.class || state.journey.travelClass || "?"); }
  function formatTime(epochMs) { const d = new Date(epochMs); return d.toLocaleTimeString("en-IN", { hour12: false }) + "." + String(d.getMilliseconds()).padStart(3, "0"); }
  function timestamp() { const d = new Date(); return d.toLocaleTimeString("en-IN", { hour12: false }) + "." + String(d.getMilliseconds()).padStart(3, "0"); }
  function safeSend(message) { try { chrome.runtime.sendMessage(message, () => void chrome.runtime.lastError); } catch {} }

  function log(text, cls = "log-action") { const line = "[" + timestamp() + "] " + text; console.log("[Tatkal UI]", line); panelLog(line, cls); safeSend({ action: "bookingLog", text: line, cls }); }
  function status(stateName, text) { panelStatus(text); safeSend({ action: "contentStatus", state: stateName, text }); }
  async function recordError(code, message, detail = "") {
    const entry = { ts: new Date().toISOString(), step: state.currentStep, url: location.href, code, message: String(message || "").slice(0, 400), detail: String(detail || "").slice(0, 800) };
    try { chrome.storage.local.get(["errorLog"], data => { const errorLog = Array.isArray(data.errorLog) ? data.errorLog : []; errorLog.push(entry); chrome.storage.local.set({ errorLog: errorLog.slice(-200) }); }); } catch {}
    safeSend({ action: "errorLogged", entry }); log("[" + code + "] " + message, "log-error");
  }

  function imageToDataUrl(img) {
    if (!img) return "";
    try {
      const canvas = document.createElement("canvas"); canvas.width = img.naturalWidth || img.width || 220; canvas.height = img.naturalHeight || img.height || 80;
      const ctx = canvas.getContext("2d"); ctx.drawImage(img, 0, 0, canvas.width, canvas.height); return canvas.toDataURL("image/png");
    } catch { return img.currentSrc || img.src || ""; }
  }
  ...
  function ensurePanel() {
    ...
    panel.innerHTML = ['<div class="ta-hdr">', '<div class="ta-logo">TATKAL UI ASSISTANT</div>', '<button class="ta-close" type="button" aria-label="Close">&times;</button>', "</div>", '<div class="ta-status" id="ta-status">Ready</div>', '<div class="ta-log" id="ta-log"></div>'].join("");
    panel.querySelector(".ta-close").addEventListener("click", () => panel.remove());
    document.body.appendChild(panel);
  }
  function panelStatus(text) { const el = document.getElementById("ta-status"); if (el) el.textContent = text || ""; }
  function panelLog(text, cls) { const logEl = document.getElementById("ta-log"); if (!logEl) return; const line = document.createElement("div"); line.className = cls || "log-action"; line.textContent = text; logEl.appendChild(line); logEl.scrollTop = logEl.scrollHeight; while (logEl.children.length > 80) logEl.firstElementChild?.remove(); }
})();
